...
//    EV << "message: " << packet << endl;
    ...
    if(!resp)
    {
        EV << "MeAppBase::socketDataArrived - Message not parsed correctly" << endl;
        throw cRuntimeError ("MeAppBase::socketDataArrived - Message not parsed correctly");
    ...
    if(receivedMessage.find("Content-Length") != receivedMessage.end() && responseMessageLength == 0)
    {
        ...
        EV << "MeAppBase::socketDataArrived - responseMessageLength: " << responseMessageLength << endl;
    }
    ...
    // else not completely arrived yet
...
//    std::string packet(packet_);
    ...
    {
        if(receivingMessage){ // it is part of a message
            EV << "some body arrived: " << endl;
            receivedMessage["body"] +=  splitting[0];
        }
        else //it is message with only header (e.g 204 non content)
        {
            std::vector<std::string> line;
            std::vector<std::string> lines = lte::utils::splitString(splitting[0], "\r\n");
            std::vector<std::string>::iterator it = lines.begin();
...
//        //check if it is part of a fragmented packet or a new one
//        body   = splitting[0];
