...
void MEPlatooningService::updatePositionsAndSpeeds(){
    ...
    for(it = cars.begin(); it != cars.end(); it++)
    {
        if(it->second.hasInitialInfo == true){
            ...
            //update position
            it->second.position.x += it->second.speed.x*time_gap + 0.5 * it->second.acceleration*cos(it->second.angularPosition.alpha)*time_gap*time_gap; // non manca un 0.5?
            it->second.position.y += it->second.speed.y*time_gap + 0.5 * it->second.acceleration*sin(it->second.angularPosition.alpha)*time_gap*time_gap; // non manca un 0.5?
            //update speed
